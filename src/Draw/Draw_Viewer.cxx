...
void Draw_Viewer::SetZoom (const Standard_Integer id, const double z)
{
  if (Draw_Batch)
    return;
...
void Draw_Viewer::SetFocal (const Standard_Integer id, const double F)
{
  if (Draw_Batch) return;
  if (myViews[id])
...
double Draw_Viewer::Zoom (const Standard_Integer id) const
{
...
double Draw_Viewer::Focal (const Standard_Integer id) const
{
  if (Draw_Batch) return 1.;
  if (myViews[id])
  ...
  case DRAW :
    {
      ...
      if (Draw_Bounds) {
	if (pt.X() + moveX > xmax) xmax = pt.X().getValue();
	if (pt.X() + moveX < xmin) xmin = pt.X().getValue();
	if (-pt.Y() - moveY > ymax) ymax = -pt.Y().getValue();
	if (-pt.Y() - moveY < ymin) ymin = -pt.Y().getValue();
      }
    }
    break;
...
void Draw_Display::MoveTo (const gp_Pnt2d& pp)
{
  ...
  case DRAW :
    PtCur = pt;
    if (Draw_Bounds) {
      if (pt.X() > xmax) xmax = pt.X().getValue();
      if (pt.X() < xmin) xmin = pt.X().getValue();
      if (pt.Y() > ymax) ymax = pt.Y().getValue();
      if (pt.Y() < ymin) ymin = pt.Y().getValue();
    }
    break;
...
//purpose  :
//=======================================================================
inline  Standard_Integer CalculRegion(const double x,
				      const double y,
				      const double x1,
				      const double y1,
				      const double x2,
...
void Draw_Display::DrawTo (const gp_Pnt2d& pp2)
{
  ...
  case DRAW : {
    ...
    if (Draw_Bounds) {
      if (p2.X() > xmax) xmax = p2.X().getValue();
      if (p2.X() < xmin) xmin = p2.X().getValue();
      if (p2.Y() > ymax) ymax = p2.Y().getValue();
      if (p2.Y() < ymin) ymin = p2.Y().getValue();
    }
  }
    break;
