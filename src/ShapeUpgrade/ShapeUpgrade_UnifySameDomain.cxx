    ...
    for (;;) // collect pcurves of a contour
    {
      ...
      for (; itl.More(); itl.Next())
      {
        ...
        if (Abs(anOffset) > thePeriod / 2)
        {
          ...
          gp_Vec2d aVec;
          if (theIndCoord == 1)
            aVec.SetCoord(anOffset, (Standard_Real)0.);
          else
            aVec.SetCoord((Standard_Real)0., anOffset);
                                 ...
                                 const Handle(Geom_Surface)& theS2)
{
  ...
  const Standard_Real aPTol = Precision::PConfusion();
...
void ShapeUpgrade_UnifySameDomain::UnifyEdges()
{
  ...
  const Standard_Real aPrec = Precision::Confusion();
