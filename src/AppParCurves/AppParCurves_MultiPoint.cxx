...
void AppParCurves_MultiPoint::Dump(Standard_OStream& o) const
{
  ...
  if (aNbPnts3D > 0)
  {
    for (Standard_Integer i = tabPoint->Lower(); i <= tabPoint->Upper(); i++)
    {
      ...
      o << " Pole x = " << getPrimal((tabPoint->Value(i) /*->Point(j)*/).X()) << std::endl;
      o << " Pole y = " << getPrimal((tabPoint->Value(i) /*->Point(j)*/).Y()) << std::endl;
      o << " Pole z = " << getPrimal((tabPoint->Value(i) /*->Point(j)*/).Z()) << std::endl;
    }
  }
  ...
  if (aNbPnts2D > 0)
  {
    for (Standard_Integer i = tabPoint2d->Lower(); i <= tabPoint2d->Upper(); i++)
    {
      ...
      o << " Pole x = " << getPrimal((tabPoint2d->Value(i) /*->Point2d(j)*/).X()) << std::endl;
      o << " Pole y = " << getPrimal((tabPoint2d->Value(i) /*->Point2d(j)*/).Y()) << std::endl;
    }
  }
}
