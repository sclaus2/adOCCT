...
Standard_Boolean ShapeFix_Face::FixMissingSeam()
{
  ...
  else if (w2.IsNull())
  {
    ...
    if (ismodeu && anIsDegeneratedTor)
    {
      ...
      p.SetCoord((Standard_Real)0.0, (ismodeu > 0 ? Standard_Real(M_PI + aPhi) : aPhi));

      Standard_Real aXCoord = -ismodeu;
      d.SetCoord(aXCoord, (Standard_Real)0.);
      aRange = 2. * M_PI;
    }
    ...
    else if (ismodev && mySurf->Surface()->IsKind(STANDARD_TYPE(Geom_BSplineSurface)))
    {
      ...
      p.SetCoord(uCoord, (ismodev < 0 ? (Standard_Real)0. : VRange));
